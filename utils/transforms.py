...
def box_transform(box, src_format, tgt_format):
    assert src_format in ["xywh", "xyxy", "cxcywh"]
    assert tgt_format in ["xywh", "xyxy", "cxcywh"]
    assert type(box) is torch.Tensor
    b0, b1, b2, b3 = box.unbind(-1)
    if src_format is "xyxy":
        b2, b3 = (b2 - b0), (b3 - b1)  # x1 -> w = (x1 - x0), y1 -> nheads = (y1 - y0)
    elif src_format is "cxcywh":
        b0, b1 = b0 - b2 / 2.0, b1 - b3 / 2.0  # cx -> x = (cx - w/2), cy -> y = (cy - nheads/2)
    if tgt_format is "xywh":
        return torch.stack([b0, b1, b2, b3], dim=-1)
    elif tgt_format is "xyxy":
        return torch.stack([b0, b1, b2 + b0, b3 + b1], dim=-1)
    elif tgt_format is "cxcywh":
        return torch.stack([b0 + b2 / 2.0, b1 + b3 / 2.0, b2, b3], dim=-1)
...
class RandomSizeCrop(object):
    ...
    def __call__(self, input_dict):
        ...
        while num_try < self.max_try:
            ...
            box_xywh = box_transform(box, src_format="xyxy", tgt_format="xywh")
...
class NormalizeAndPad(object):
    ...
    def __call__(self, input_dict):
        ...
        if 'box' in input_dict.keys():
            ...
            h, w = out_img.shape[-2:]
            # box = xyxy2xywh(box)
            box = box_transform(box, src_format="xyxy", tgt_format="xywh")
            box = box / torch.tensor([w, h, w, h], dtype=torch.float32)
            # FIXME: Note: Make Normalization!!!!
            input_dict['box'] = box
