...
from loss import calc_first_loss
from metrics import calc_first_metric
import models
...
class FirstStageRunner:
    ...
    def _init_model(self, model_config):
        ...
        print('FirstStageRunner model: ', self.model, model_config)
    ...
    def _train_one_epoch(self, epoch, last_total_step):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.train_loader, 1):
            ...
            loss, loss_items = calc_first_loss(outputs=output, targets=label)
            loss.backward()
            # update
            self.optimizer.step()
    ...
    def eval(self, epoch, data):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(data_loader, 1):
                ...
                loss, _ = calc_first_loss(outputs=output, targets=label)
                all_loss += -loss.item() * batch_size
                all_num += batch_size
                all_pred.extend(output[0].max(-1)[1])
                all_real.extend(label)
                # metrics = calc_first_metric(label_logit=move_to_cpu(output[0]),
                #                             label=move_to_cpu(label))
        ...
        all_pred, all_real = torch.tensor(all_pred), torch.tensor(all_real)
        metrics = calc_first_metric(all_pred, all_real)
        neg_loss = all_loss / all_num
        self._print_metrics(epoch, metrics["item"], data)
